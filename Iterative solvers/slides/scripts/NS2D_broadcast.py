...
class NS2D:
    ...
    def __init__(self, lx=2*np.pi, ly=2.*np.pi, nu=5e-4, nx=128, ny=128, T=10., iostep=10, sparsity=None, antialiasing=False):
        ...
        self.dt = 0.25*min(self.dx, self.dy) # NOTE: Suggested by Sam Taira.
    ...
    def plot_figure(self, fig, omega_hat, cmap=plt.cm.RdBu):
        '''
        ...
        if omega.min() * omega.max() < 0:
            ax.pcolormesh(self.xp, self.yp, omega_p, vmin=-amp, vmax=amp, shading='gouraud', cmap=cmap)
        else:
...
if __name__ == '__main__':
    ...
    nx, ny = 256, 256
    simulation = NS2D(nx=256, ny=256, lx=1, ly=0.5625, nu=5e-4, iostep=16, T=1, antialiasing=True)

    # --> Sets-up the initial condition.
    # omega_hat = simulation.initial_condition(case=3, nmax=64, random_state=0)
    omega_hat = np.random.randn(nx, ny)
    omega_hat -= omega_hat.mean()
    ...
    fig = plt.figure(figsize=(8, 4.5), facecolor="black")
    camera = Camera(fig)
    ...
    anim = camera.animate()
    anim.save(
        "../imgs/2D_turbulence_fullsize.mp4",
        fps=30, extra_args=["-vcodec", "libx264"]
        )

    plt.savefig("../imgs/2D_turbulence_fullsize.png", bbox_inches="tight", dpi=300)
